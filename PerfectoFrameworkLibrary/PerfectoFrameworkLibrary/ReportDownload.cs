...
namespace ExportAPI
{
    ...
    {
        public static void downloadFile(string fileName, System.Net.HttpWebRequest request, string suffix, string description, string location)
        {
            ...
            try
            {
                if (HttpStatusCode.OK == response.StatusCode)
                {
                    ...
                    using (Stream output = File.OpenWrite(location + "\\" +path))
        ...
        public static void downloadVideo(JToken testExecution, string location)
        {
            ...
            request.Headers.Add("PERFECTO_AUTHORIZATION", Reporting.OFFLINE_TOKEN);
            downloadFile(id, request, format, "Test Execution Video", location);
        }

        internal static void downloadAttachments(JToken testExecution, string location)
        {
            try
            {
                ...
                if (artifacts.Count() > 0)
                {
                    foreach (var currArtifact in artifacts)
                    {
                        ...
                        if (type.Equals("DEVICE_LOGS"))
                        {
                            ...
                            request.Headers.Add("PERFECTO_AUTHORIZATION", Reporting.OFFLINE_TOKEN);
                            downloadFile(testId, request, ".zip", "Device Logs", location);
                        }
                    }
                }
            }
